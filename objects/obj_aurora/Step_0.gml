...
if (speed != 0) {
	// Caso esteja movendo
	if (fade_wait != 50) {
		audio_stop_sound(running_audio)
	}
	if (!audio_is_playing(running_audio)) {	
		var _audio_offeset = random(audio_sound_length(snd_running))
		var _pitch = 0.95 + random(0.1)
		running_audio = audio_play_sound(snd_running, 0, true, 10, _audio_offeset, _pitch)
		fade_wait = 50
	}
} else {
	// Caso parado
	if (fade_wait == 50) {
		audio_sound_gain(running_audio, -10, 200)
	}
	if (fade_wait == 0) {
		audio_stop_sound(running_audio)
	}
